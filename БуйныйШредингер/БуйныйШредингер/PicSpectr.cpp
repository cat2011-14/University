...
IMPLEMENT_DYNAMIC(PicSpectr, CStatic)
ULONG_PTR tokenSpectr;
PicSpectr::PicSpectr()
{
	GdiplusStartupInput input;
	Status s;
	s = GdiplusStartup(&tokenSpectr, &input, NULL);
	if (s != Ok) {
...
PicSpectr::~PicSpectr()
{
	GdiplusShutdown(tokenSpectr);
}


BEGIN_MESSAGE_MAP(PicSpectr, CStatic)
END_MESSAGE_MAP()



// обработчики сообщений PicSpectr

void PicSpectr::Масштабирование(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
REAL PicSpectr::KoordX(double X)
{
	return (REAL)(Kx*(X - Xmin));
}

REAL PicSpectr::KoordY(double Y)
{
	return (REAL)(Ky*(Y - Ymax));
}

void PicSpectr::Параметры(vector<double>Данные, double R0max)
{
	Xmax = R0max;
	Xmin = 0.0;
...
void PicSpectr::Прорисовка(Graphics &БитКарта)
{
...
void PicSpectr::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
